...
VAPT is split into 14 specialization folders — **Methodology_and_Standards**, **Reconnaissance_and_OSINT**, **Network_Pentesting**, **Web_Application_Pentesting**, **API_Security_Testing**, **Active_Directory_and_Internal_Pentesting**, **Mobile_Application_Pentesting**, **Wireless_Pentesting**, **Cloud_Pentesting**, **Post_Exploitation_and_Privilege_Escalation**, **Exploit_Development**, **Tools_and_Cheatsheets**, **Hardware_for_Hacking**, and **Reporting_and_Risk_Scoring** — each with its own `00_Roadmap.md` (or overview note). This file is the order to move through the folders themselves: building from *how an engagement is run*, through each attack surface in the order a real engagement typically touches them, to *how findings get written up*. Two folders — Tools_and_Cheatsheets and Hardware_for_Hacking — are reference/equipment folders rather than a strict learning sequence, so they're placed late and can be dipped into any time. This folder assumes CS-Fundamentals is already complete — it leans on Networking, Security Basics ([[Security_Testing_Types]], [[Vulnerability_Management]]), Linux, and Windows throughout without re-explaining them.
...
→ Start at [[Nmap_Fundamentals_and_Scanning_Techniques]], finish at [[Firewall_IDS_IPS_Evasion_and_Stealth_Scanning]].
...
→ Start at [[Web_App_Recon_and_Fingerprinting]], finish at [[Web_App_Exploitation_Workflow]].
...
→ Start at [[API_Recon_and_Fingerprinting]], finish at [[API_Security_Assessment_Workflow]].
...
→ Start at [[AD_Enumeration_and_LDAP]], finish at [[AD_Attack_Chain_Workflow]].
...
→ Start at [[Mobile_App_Recon_and_Testing_Setup]], finish at [[Mobile_App_Assessment_Workflow]].
...
→ Start at [[Wireless_Fundamentals_and_Monitor_Mode]], finish at [[Wireless_Pentest_Workflow]].
...
## Folder 12 — Tools_and_Cheatsheets 🟡 (in progress)
...
→ Currently has the two-part software inventory: [[Software_Toolkit_Part1_Recon_Network_Passwords]] and [[Software_Toolkit_Part2_Exploitation_Web_AD_Wireless]] — per-tool command cheatsheets to follow.

## Folder 13 — Hardware_for_Hacking 🟡 (in progress)

**Why here:** a physical-equipment shopping and reference guide — the gear used across wireless, physical-access, and hands-on engagements. Placed alongside Tools_and_Cheatsheets since it's reference material, not a conceptual learning sequence.

→ Start at [[00_Overview]], covering [[Wireless_and_RF_Hardware]], [[Physical_Access_and_Payload_Devices]], and [[Computing_and_Storage_Accessories]].

## Folder 14 — Reporting_and_Risk_Scoring (Capstone)
...
Methodology_and_Standards → Reconnaissance_and_OSINT → Network_Pentesting → Web_Application_Pentesting → API_Security_Testing → Active_Directory_and_Internal_Pentesting → Mobile_Application_Pentesting → Wireless_Pentesting → Cloud_Pentesting → Post_Exploitation_and_Privilege_Escalation → Exploit_Development → Tools_and_Cheatsheets → Hardware_for_Hacking → Reporting_and_Risk_Scoring

---
*✅ = complete. 🟡 = in progress. Folders 9-11 and 14 currently exist as empty subfolders — content to follow. Each completed/in-progress folder has its own roadmap or overview note with the full reading order.*
